...
#endif
#include <iostream>
class MM2hdmi_api_t: public sim_api_t<VerilatorDataWrapper*> {
    public:
    MM2hdmi_api_t(VMM2hdmi* _dut) {
        dut = _dut;
        main_time = 0L;
        is_exit = false;
#if VM_TRACE
    ...
    void init_sim_data() {
        ...
        sim_data.inputs.push_back(new VerilatorCData(&(dut->io_newData)));
        sim_data.inputs.push_back(new VerilatorSData(&(dut->io_data)));
        sim_data.outputs.push_back(new VerilatorCData(&(dut->io_hSync)));
        sim_data.outputs.push_back(new VerilatorCData(&(dut->io_vSync)));
        sim_data.outputs.push_back(new VerilatorCData(&(dut->io_green)));
        sim_data.outputs.push_back(new VerilatorCData(&(dut->io_blue)));
        sim_data.outputs.push_back(new VerilatorCData(&(dut->io_red)));
        sim_data.signals.push_back(new VerilatorCData(&(dut->reset)));
        sim_data.signal_map["MM2hdmi.reset"] = 0;
    }
#if VM_TRACE
...
int main(int argc, char **argv, char **env) {
    ...
    VMM2hdmi* top = new VMM2hdmi;
    std::string vcdfile = "test_run_dir/mm2hdmi.MM2hdmiTester1903281006/MM2hdmi.vcd";
